...
Status: EXECUTING IDENTITY CONFIRMED; H1 remains open pending native extension/install characterization (H1.2–H1.5).
...
| Banner `upstream` label | volatile local `origin/main`; values observed across planning include `ccc92c52`, `b963d323`, and `f171842f` | Pinned-source `banner.py` proves this label reads `origin/main`; it is drift information, not executing-code identity. No H1 probe fetch is permitted. |
...
1. The candidate checkout was 2,004 commits behind the existing local `origin/main` ref at H1.1; this is deliberate pinning, not permission to update during the MVP.
